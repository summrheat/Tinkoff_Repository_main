    ...
    public int process(List<Update> updates) {
            updates.forEach(update ->{
                ...
                switch (msg){
                    ...
                    case "/help" -> {
                        bot.execute(new SendMessage(update.message().chat().id(), "список команд: 1.start 2.help 3.track 4.untrack 5.list"));
                    }
                    case "/track" -> {
                        bot.execute(new SendMessage(update.message().chat().id(), "введите ссылку для отслеживания"));
                        update.message().messageId();
                        updateid_fromComand = update.message().messageId();
                        comand = msg;
                    }
                    case "/untrack" -> {
                        bot.execute(new SendMessage(update.message().chat().id(), "введите ссылку для отключения отслеживания"));
                        updateid_fromComand = update.message().messageId();
                        comand = msg;
                    }
                    case "/list" -> {
                    ...
                    default ->{
                        ...
                        else bot.execute(new SendMessage(update.message().chat().id(), "Такой команды не существует"));
                    }
        ...
        void start(Update update){
            ...
            bot.execute(new SendMessage(update.message().chat().id(), "начало работы бота"));
        }
